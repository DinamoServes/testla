...
export default function AdminServerDetailPage() {
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      <div className="mb-2"><b>Owner:</b> {server.owner?.username || server.owner?.email || 'N/A'}</div>
      ...
      {stats && (
        <div className="mb-2">
          ...
          {stats.networks && Object.keys(stats.networks).length > 0 && (
            <span> | Network: {Object.entries(stats.networks).map(([iface, net]) => {
              const n = net as { tx_bytes?: number; rx_bytes?: number };
              return `${iface}: ↑${((n.tx_bytes || 0)/1024).toFixed(1)}KB ↓${((n.rx_bytes || 0)/1024).toFixed(1)}KB`;
            }).join(', ')}</span>
          )}
        </div>
      )}
